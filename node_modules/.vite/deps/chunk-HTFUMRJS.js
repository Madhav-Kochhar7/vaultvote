...
function hash(authorization, options = {}) {
  ...
  return keccak256(concat("0x05", fromHex2(toTuple2(presign ? {
...
function fromHex4(data) {
  return fromBytes2(fromHex(data));
}
...
function format(address, options = {}) {
  ...
  const data = concat2(version, zone, fromHex(address));
...
function from9(value, decimals = 0) {
...
function fromEther(ether, unit = "wei") {
  return from9(ether, exponents.ether - exponents[unit]);
}
function fromGwei(gwei, unit = "wei") {
  return from9(gwei, exponents.gwei - exponents[unit]);
}
...
function getSignPayload3(envelope, options = {}) {
  ...
  if (options.from)
    return keccak256(concat("0x04", sigHash, options.from));
  return sigHash;
}
...
//# sourceMappingURL=chunk-HTFUMRJS.js.map
